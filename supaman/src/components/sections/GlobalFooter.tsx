import styles from "./GlobalFooter.module.css";
import navStyles from "../../styles/NavLinks.module.css";

export default function GlobalFooter() {
  return (
    <>
      <footer className={styles.footer} data-color-scheme="dark">
        <div className={`${styles.contentWrapper} ${styles.content}`}>
          <div className={`${styles.column} ${styles.gap6x} ${styles.logo}`}>
            <svg xmlns="http://www.w3.org/2000/svg" height="25" width="168" fill="currentColor">
              <use href="#superhuman-logo"></use>
            </svg>
            <h3 className={`text_text__RO8_0 ${styles.logoTitle}`}>
              Superpowers,
              <br />
              everywhere you work
            </h3>
          </div>
          <nav className={styles.contentNav}>
            <div className={styles.contentItem}>
              <h3
                className={`text_text__RO8_0 text_fontWeightSemibold__Qj9hZ ${styles.contentItemTitle}`}
              >
                Products
              </h3>
              <ul className={`${styles.column} ${styles.gap4x}`}>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/products/go-ai-assistant"
                  >
                    Superhuman Go
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/products/grammarly"
                  >
                    Grammarly
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/products/coda"
                  >
                    Coda
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="https://superhuman.com/products/mail"
                  >
                    Superhuman Mail
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="https://superhuman.com/store/agents"
                  >
                    Agent Store
                  </a>
                </li>
              </ul>
            </div>
            <div className={styles.contentItem}>
              <h3
                className={`text_text__RO8_0 text_fontWeightSemibold__Qj9hZ ${styles.contentItemTitle}`}
              >
                Company
              </h3>
              <ul className={`${styles.column} ${styles.gap4x}`}>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/contact"
                  >
                    Contact Us
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/support"
                  >
                    Help Center
                  </a>
                </li>
                <li>
                  <a
                    href="https://job-boards.greenhouse.io/grammarly"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    Careers
                    <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11">
                      <use href="#open-new-icon"></use>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="https://superhuman.com/partners"
                  >
                    Partners
                  </a>
                </li>
              </ul>
            </div>
            <div className={styles.contentItem}>
              <h3
                className={`text_text__RO8_0 text_fontWeightSemibold__Qj9hZ ${styles.contentItemTitle}`}
              >
                Legal
              </h3>
              <ul className={`${styles.column} ${styles.gap4x}`}>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/legal/terms"
                  >
                    Terms
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.grammarly.com/privacy-policy"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    Privacy Policy
                    <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11">
                      <use href="#open-new-icon"></use>
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/legal/trust"
                  >
                    Trust
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                    href="/legal/terms/customer-business-agreement"
                  >
                    Customer Business Agreement
                  </a>
                </li>
                <li>
                  <a
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink} ${styles.privacyChoicesNavLink}`}
                    href="/render#"
                  >
                    Cookies Preferences Center
                  </a>
                </li>
              </ul>
            </div>
            <div className={styles.contentItem}>
              <h3
                className={`text_text__RO8_0 text_fontWeightSemibold__Qj9hZ ${styles.contentItemTitle}`}
              >
                Connect
              </h3>
              <ul className={`${styles.column} ${styles.gap4x}`}>
                <li>
                  <a
                    href="https://www.linkedin.com/company/superhumanhq"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="13"
                      height="13"
                      className={navStyles.navLinkBeforeIcon}
                      aria-hidden="true"
                    >
                      <use href="#linkedin-icon"></use>
                    </svg>
                    LinkedIn
                  </a>
                </li>
                <li>
                  <a
                    href="https://x.com/Superhuman"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="13"
                      height="12"
                      className={navStyles.navLinkBeforeIcon}
                      aria-hidden="true"
                    >
                      <use href="#x-icon"></use>
                    </svg>
                    X
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.tiktok.com/@superhumanhq?lang=e"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="14"
                      height="14"
                      className={navStyles.navLinkBeforeIcon}
                      aria-hidden="true"
                    >
                      <use href="#tiktok-icon"></use>
                    </svg>
                    TikTok
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.instagram.com/superhuman__hq/?hl=en"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="14"
                      height="14"
                      className={navStyles.navLinkBeforeIcon}
                      aria-hidden="true"
                    >
                      <use href="#instagram-icon"></use>
                    </svg>
                    Instagram
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.threads.com/@superhuman__hq"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="14"
                      height="14"
                      className={navStyles.navLinkBeforeIcon}
                      aria-hidden="true"
                    >
                      <use href="#threads-icon"></use>
                    </svg>
                    Threads
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.facebook.com/SuperhumanCo"
                    className={`link_link__AAIEV ${navStyles.navLink} ${styles.navLink}`}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="14"
                      height="14"
                      className={navStyles.navLinkBeforeIcon}
                      aria-hidden="true"
                    >
                      <use href="#facebook-icon"></use>
                    </svg>
                    Facebook
                  </a>
                </li>
              </ul>
            </div>
          </nav>
          <p className={styles.trademark}>Builders of Superhuman Platform® apps</p>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            role="none"
            viewBox="0 0 343 33"
            fill="currentColor"
            className={styles.wordmark}
          >
            <path d="M187.952 12.5855V0.713135H195.114V31.84H187.952V19.0752H175.276V31.84H168.115V0.71318H175.276V12.5856L187.952 12.5855Z"></path>
            <path d="M0.0115874 22.1911H7.17332C7.66466 25.0829 9.8392 26.5063 13.7534 26.5063C17.3566 26.5063 19.3561 25.1282 19.3561 22.7731C19.3561 20.9486 18.4243 20.1072 15.7978 19.5252L11.1269 18.5031C3.52446 16.8144 0.587614 14.2329 0.587614 9.16099C0.587614 3.60329 5.52391 -0.000244141 13.0811 -0.000244141C20.6382 -0.000244141 25.6198 3.46765 25.9305 9.692H18.8141C18.3227 7.29151 16.3683 6.04344 13.0754 6.04344C9.78264 6.04344 7.73812 7.24618 7.73812 9.33629C7.73812 10.9345 8.62483 11.6462 11.7427 12.4032L16.1423 13.3802C23.8804 15.0237 26.5066 17.4695 26.5066 22.9875C26.5066 28.9012 21.6606 32.5497 13.7421 32.5497C5.42799 32.5497 0.310723 28.7261 0 22.1854L0.0115874 22.1911Z"></path>
            <path d="M53.4225 0.713135H60.5842V18.6345C60.5842 28.0614 56.139 32.5517 46.9778 32.5517C37.8166 32.5517 33.3716 28.0614 33.3716 18.6345V0.713135H40.5333V18.8099C40.5333 23.8818 42.4875 26.1013 46.9834 26.1013C51.4793 26.1013 53.4337 23.8762 53.4337 18.8099V0.713135H53.4225Z"></path>
            <path d="M125.297 31.84H101.817V0.713135H125.297V7.11818H108.973V12.6759H122.828V19.0809H108.973V25.4406H125.297V31.8456V31.84Z"></path>
            <path d="M223.705 0.713135H230.866V18.6345C230.866 28.0614 226.421 32.5517 217.26 32.5517C208.099 32.5517 203.654 28.0614 203.654 18.6345V0.713135H210.815V18.8099C210.815 23.8818 212.77 26.1013 217.266 26.1013C221.762 26.1013 223.717 23.8762 223.717 18.8099V0.713135H223.705Z"></path>
            <path d="M255.407 15.6132L263.009 0.713379H271.415V31.8402H264.434V11.253L256.203 27.5759H254.604L246.376 11.253V31.8402H239.396V0.713379H247.798L255.401 15.6132H255.407Z"></path>
            <path d="M308.362 31.8386H301.248L300.09 23.8804H287.325L286.167 31.8386H279.05L281.896 12.1377C283.273 3.24221 286.386 0 293.684 0C300.981 0 304.133 3.24783 305.465 12.1377L308.359 31.8386H308.362ZM288.308 17.3906H299.113L298.446 12.7197C297.732 8.00347 296.577 6.45033 293.69 6.45033C290.805 6.45033 289.685 8.00908 288.975 12.7197L288.308 17.3906Z"></path>
            <path d="M336.016 20.9902V0.713379H343V31.8402H335.836L322.988 11.5633V31.8402H316.007V0.713379H323.168L336.016 20.9902Z"></path>
            <path d="M148.984 0.638184C151.183 0.638234 153.114 1.04691 154.778 1.86377C156.442 2.68151 157.735 3.85521 158.656 5.38525C159.577 6.91585 160.038 8.72098 160.038 10.8003C160.038 12.8798 159.576 14.445 158.655 15.9751C157.734 17.5058 156.063 18.784 154.343 19.5034C154.035 19.6322 153.373 19.8604 152.484 20.1538L160.442 31.811H151.905L145.549 22.3481C143.977 22.8343 142.431 23.3064 141.134 23.7026V31.8384H133.971V0.638184H148.984ZM141.134 16.7144C144.477 15.866 149.227 14.6403 150.344 14.2271C151.117 13.9413 153.073 13.2905 153.073 10.8003C153.073 7.82439 150.123 7.54642 148.262 7.54639H141.134V16.7144Z"></path>
            <path d="M69.1135 0.638184H84.1256C86.3243 0.638184 88.2555 1.04684 89.9199 1.86375C91.5837 2.68148 92.8763 3.85497 93.7976 5.38503C94.7185 6.91574 95.1794 8.72094 95.1794 10.8005C95.1794 12.88 94.7176 14.4448 93.7968 15.9749C92.8755 17.5056 91.2059 18.7841 89.4849 19.5035C87.6234 20.2817 72.8462 24.7471 72.8462 24.7471L72.8577 17.5761C72.8577 17.5761 83.7177 14.8807 85.4862 14.2268C86.2588 13.9411 88.2145 13.2908 88.2145 10.8005C88.2145 7.82417 85.2649 7.54663 83.404 7.54663H74.2695L76.2753 5.54082V31.8382H69.1135V0.638184Z"></path>
          </svg>
        </div>
      </footer>
    </>
  );
}
